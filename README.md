...
- **Terms of service.** Using third-party APIs or AI models in the manner
  this project demonstrates may violate the terms of service of those API
  or model providers, now or in the future. Use this software at your own
  discretion and risk; the author(s) accept no liability for any
  consequences, including account suspension or API access revocation.
- **Provider support.** This project has only been tested with OpenRouter.
  The other provider integrations (`openai`, `anthropic`, `google`,
  `vertexai`, `zai`, `zai_coding`) are provided as-is and may break. If you
  hit a problem with a non-OpenRouter provider, please open an issue, or
  fork the project and fix it yourself.

## License

This project (source code, configuration, and prompt files such as
`chat.md` and `aggregator.md`) is licensed under the
[GNU Affero General Public License v3.0](LICENSE) or later.

The license does **not** extend to the *output* of the program — text
generated by the chat or aggregator models during a session (responses,
fabricated search results, logs, transcripts). That output is not a
derivative work of this software and may be used under whatever terms you
choose. See `NOTICE.md` for details.

## Contributing

Contributions are welcome, particularly:

- Fixes for provider integrations other than OpenRouter, which are
  untested (see Disclaimers above).
- Bug fixes, test coverage, and documentation improvements.
- New research scenarios or prompt variants under `docs/`.

To contribute, open an issue describing the bug or proposal, or fork the
repo and submit a pull request. Please run `python -m pytest` before
submitting. By contributing, you agree your contributions will be
